...
//Match timer
function timerCount() {
    if (timeRemaining == 26 || timeRemaining == 36) {
        switchSound.play(); // Play switch side sounds
    }
    if (timeRemaining <= 1 && !inCountdown) { // Regular countdown
        timerStop();
        timerText.innerHTML = "TIME UP";
        timerReady = true;
    } else if (timeRemaining <= 1 && inCountdown) { // Enabled countdown
        startSound.play();
        inCountdown = false;
        timeRemaining = 61; // Return to regular time
    }
    if (inCountdown) { // Play coundown sounds
        shortBeep.play();
    }
    timeRemaining -= 1;
    if (timeRemaining <=60) {
...
function switchCountdown() {
    if (timerReady) {
        countdownEnabled = countdownEnabled ? false : true;
        if (countdownEnabled) {
            countdownSwitch.innerHTML = "Disable Countdown"
        } else {
            countdownSwitch.innerHTML = "Enable Countdown"
        }
    }
}
